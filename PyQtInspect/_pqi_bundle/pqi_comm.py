...
from PyQtInspect.pqi_log import logger
...
from PyQtInspect._pqi_bundle.pqi_comm_constants import (
    ...
    CMD_SHOW_RETURN_VALUES, CMD_SET_UNIT_TEST_DEBUGGING_MODE, CMD_INPUT_REQUESTED, CMD_GET_DESCRIPTION,
    CMD_PROCESS_CREATED,
...
#             xml.write("<xml>")
#             _typeName, val_dict = pydevd_vars.resolve_compound_variable_fields(self.thread_id, self.frame_id,
#                                                                                self.scope, self.attributes,
#                                                                                dbg.get_user_type_renderers())
#             if val_dict is None:
#                 val_dict = {}
#
...
#                                                 user_type_renderers=dbg.get_user_type_renderers()))
#
#             xml.write("</xml>")
...
#                                                      self.attr, self.expression, get_exception_traceback_str()))
#             dbg.writer.add_command(cmd)
#
#
# # =======================================================================================================================
...
#                 xml = "<xml>"
#                 xml += pydevd_xml.frame_vars_to_xml(frame.f_locals, self.group_type, hidden_ns,
#                                                     dbg.get_user_type_renderers())
#                 del frame
#                 xml += "</xml>"
...
#                 cmd = dbg.cmd_factory.make_error_message(self.sequence, "Frame not found: %s from thread: %s" % (
#                 self.frame_id, self.thread_id))
#                 dbg.writer.add_command(cmd)
#         except:
#             cmd = dbg.cmd_factory.make_error_message(self.sequence, "Error resolving frame: %s from thread: %s" % (
#             self.frame_id, self.thread_id))
#             dbg.writer.add_command(cmd)
#
#
...
#             cmd = dbg.cmd_factory.make_error_message(self.sequence,
#                                                      "Error getting smart step into veriants for frame: %s from thread: %s"
#                                                      % (self.frame_id, self.thread_id))
...
#             if self.temp_name != "":
#                 pydevd_vars.change_attr_expression(self.thread_id, self.frame_id, self.temp_name, self.expression, dbg,
#                                                    result)
#             xml = "<xml>"
...
#     def do_it(self, dbg):
#         try:
#             cmd = dbg.cmd_factory.make_send_curr_exception_trace_message(self.sequence, self.thread_id,
...
#
#                 result, exception_occurred = pydevd_console_integration.console_exec(self.thread_id, self.frame_id,
#                                                                                      self.expression, dbg)
#                 xml = "<xml>"
#                 xml += pydevd_xml.var_to_xml(result, "")
