...
def _make_tooltip_class(qt_module):
    ...
    class DetailedInspectTooltip(QWidget):
        ...
        def setInfo(self, info: _WidgetInfo):
            # this function will generate multiple widgets, so we need to suppress the patch
...
class DetailedInspectTooltipManager:
    def __init__(self, qt_module):
        QtWidgets = qt_module.QtWidgets
        QWidget = QtWidgets.QWidget
        self._QWidget_cls = QWidget
        self._tooltipCls = _make_tooltip_class(qt_module)
        self._tooltip = None

    def show_tooltip(self, target_widget):
        ...
        if self._tooltip is None:
            with SuppressPatchMark.marked(
                self._QWidget_cls  # patch QWidget is enough, because all the other widgets are subclasses of QWidget
            ):
                self._tooltip = self._tooltipCls()
        self._tooltip.showNear(target_widget)
