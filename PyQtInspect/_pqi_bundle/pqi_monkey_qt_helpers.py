...
def patch_QtWidgets(QtModule, qt_support_mode='auto', is_attach=False):
    ...
    def _createHighlightFg(parent: QtWidgets.QWidget):
        # Create new instance first, then call the previous __init__
    ...
    class EventListener(QtCore.QObject):
        ...
        def _handleEnterEvent(self, obj, event):
            ...
            if _entered_widget_stack:
                # If there are elements at the top of the stack, cancel the selection state of the top element
                # Otherwise QTabWidget will behave abnormally
                # todo see if it can be integrated on the stack, after all they are strongly associated
        ...
        def _handleLeaveEvent(self, obj, event):
            # Note this asymmetry
            # Because leaveEvent is triggered when the mouse leaves, but when the mouse leaves, it's possible that the mouse has already entered the next widget
            # So cannot directly pop
            if not _is_inspect_enabled():
                return
        ...
        def _handleMouseReleaseEvent(self, obj, event) -> bool:
            """ Handle mouse click events, need to return a bool value here to indicate whether to intercept the event """
            if not _is_inspect_enabled():
                return False
            ...
            # Events sent by ourselves through postEvent, don't handle
            # Don't use event.spontaneous() to judge, because for QTextBrowser click events, spontaneous() returns False
            ...
            if event.button() != MouseButtonEnum.LeftButton:
                if debugger is not None and debugger.mock_left_button_down and event.button() == MouseButtonEnum.RightButton:
                    ...
                    # Use postEvent to propagate the event, instead of directly calling obj.mousePressEvent, so that other eventFilters can receive this event
        ...
        def _handleMousePressEvent(self, obj, event):
            ...
            # For the currently inspected widget, prevent MousePress event propagation
            # Prevent some eventFilters from changing the inspected widget when handling MousePress events after clicking, causing subsequent MouseRelease event handling to be abnormal
            return True
    ...
    # For PyQt, only need to patch the base class QWidget
    # But for PySide, need to patch all QWidget subclasses
